...
class billClass:
    def __init__(self,root):
        ...
        self.chk_print = 0
        self._setup_window()
        self._setup_product_frame()
        self._setup_customer_frame()
        self._setup_calculator_and_cart()
        self._setup_add_cart_widgets()
        self._setup_billing_area()
    ...
    def _setup_customer_frame(self):
    ...
    def _setup_calculator_and_cart(self):
        ...
        for i, (text, cmd) in enumerate([
            (7, lambda: self.get_input(7)), (8, lambda: self.get_input(8)), (9, lambda: self.get_input(9)), ('+', lambda: self.get_input('+')),
            (4, lambda: self.get_input(4)), (5, lambda: self.get_input(5)), (6, lambda: self.get_input(6)), ('-', lambda: self.get_input('-')),
            (1, lambda: self.get_input(1)), (2, lambda: self.get_input(2)), (3, lambda: self.get_input(3)), ('*', lambda: self.get_input('*')),
            (0, lambda: self.get_input(0)), ('C', self.clear_cal), ('=', self.perform_cal), ('/', lambda: self.get_input('/'))
        ]):
            row, col = divmod(i, 4)
            Button(Cal_Frame, text=text, font=('arial', 15, 'bold'), command=cmd, bd=5, width=4, pady=10 if row < 3 else 15, cursor="hand2").grid(row=row+1, column=col)
    ...
    def _setup_add_cart_widgets(self):
        ...
        Entry(Add_CartWidgets_Frame, textvariable=self.var_pname, font=("times new roman", 15), bg="lightyellow", state='readonly').place(x=5, y=35, width=190, height=22)
        Label(Add_CartWidgets_Frame, text="Price Per Qty", font=("times new roman", 15), bg="white").place(x=230, y=5)
        Entry(Add_CartWidgets_Frame, textvariable=self.var_price, font=("times new roman", 15), bg="lightyellow", state='readonly').place(x=230, y=35, width=150, height=22)
        Label(Add_CartWidgets_Frame, text="Quantity", font=("times new roman", 15), bg="white").place(x=390, y=5)
        Entry(Add_CartWidgets_Frame, textvariable=self.var_qty, font=("times new roman", 15), bg="lightyellow").place(x=390, y=35, width=120, height=22)
    ...
    def _setup_billing_buttons(self):
        ...
        Button(billMenuFrame, text="Print", command=self.print_bill, cursor="hand2", font=("goudy old style", 15, "bold"), bg="lightgreen", fg="white").place(x=2, y=80, width=120, height=50)
        Button(billMenuFrame, text="Clear All", command=self.clear_all, cursor="hand2", font=("goudy old style", 15, "bold"), bg="gray", fg="white").place(x=124, y=80, width=120, height=50)
        Button(billMenuFrame, text="Generate Bill", command=self.generate_bill, cursor="hand2", font=("goudy old style", 15, "bold"), bg="#009688", fg="white").place(x=246, y=80, width=160, height=50)
#---------------------- all functions ------------------------------
